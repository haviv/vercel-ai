...
async function main() {
  let pokemonMcpClient: Awaited<ReturnType<typeof experimental_createMCPClient>> | undefined;
  ...
  const model = USE_OLLAMA ? ollama(OLLAMA_MODEL) : openai(OPENAI_MODEL);
  const providerName = USE_OLLAMA ? 'Ollama' : 'OpenAI';

  try {
    // Pokemon MCP Server
    const pokemonStdioTransport = new StdioClientTransport({
    ...
    pokemonMcpClient = await experimental_createMCPClient({
      transport: pokemonStdioTransport,
    });
    ...
    const pokemonTools = await pokemonMcpClient.tools({
      schemas: {
        'get-pokemon': {
    ...
    const rl = readline.createInterface({
      ...
      prompt: '\n🤖 Ask me PLC related questions (or type "exit" to quit): '
    });

    console.log('🚀 Multi-MCP AI Assistant started!');
    console.log('💡 I can help you with Pokemon questions AND database queries!');
    console.log('🔧 Available tools:');
    const allToolNames = Object.keys(tools);
    allToolNames.forEach((toolName, index) => {
    ...
    rl.on('line', async (input) => {
      ...
      try {
        ...
        const result = streamText({
          ...
          system: `You are a helpful database assistant specialized in Governance, Risk, and Compliance (GRC).  
...
Your role: transform raw identity and compliance data into clear, accurate, and business-relevant answers that support governance, compliance, and risk management decisions.  ` ,
          ...
          onStepFinish: async ({ toolResults }) => {
            ...
            //     console.log(`  ${index + 1}. ${result.toolName}: ${JSON.stringify(result.output, null, 2)}`);
            //   });
            //   console.log('');
            // }
          },
        ...
        // Wait for the stream to finish and get final results
